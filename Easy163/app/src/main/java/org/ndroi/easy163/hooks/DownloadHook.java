...
public class DownloadHook extends BaseHook {
    ...
    public void hookRequest(Request request) {
        ...
        Crypto.Request cryptoRequest = null;
        try {
            cryptoRequest = Crypto.decryptRequestBody(new String(request.getContent()));
        ...
        try {
            bytes = Crypto.encryptRequestBody(cryptoRequest).getBytes();
        } catch (BadPaddingException | IllegalBlockSizeException e) {
            e.printStackTrace();
            return;
        }
